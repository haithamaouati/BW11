...
## Usage
#### First Method
1. Burn the [Windows 11](https://www.microsoft.com/software-download/windows11) .iso :cd: on USB flash drive using [Rufus](https://rufus.ie) tool.
...
> [!NOTE]
> Merging a `.reg` file that writes to `HKEY_LOCAL_MACHINE` still requires administrator privileges, even though no explicit "Run as administrator" option appears in the ContextMenu for `.reg` files.
